...
import { kisame, kisame_ID, kisame_modifier } from './data/actors/kisame'
import { itachi, itachi_ID } from './data/actors/itachi'
...
const itachiContext = makeContextFromActor(itachi(), {
  targetActorIDs: [kisame_ID],
})

pushActors([kisame(), itachi()])
addExperience((a) => a.ID === kisame_ID || a.ID === itachi_ID, 9876)
pushActions([
...
const a = getActor(getState(), (a) => a.ID === kisame_ID)!
const b = getActor(getState(), (a) => a.ID === itachi_ID)!
...
let actor = resolveActor(getState(), kisame_ID)!
let itachiActor = resolveActor(getState(), itachi_ID)!
